...
Ext.calendar.CalendarView = Ext.extend(Ext.BoxComponent, {
            ...
            for (d = 0; d < this.dayCount; d++) {
                if (evtsInView.getCount() > 0) {
                    var evts = evtsInView.filterBy(function (rec) {
                        ...
                        var startsOnDate = (dt.getTime() == st.getTime());
                        var spansFromPrevView = (w == 0 && d == 0 && (dt > st));
    ...
    prepareEventGrid: function (evts, w, d) {
        ...
        evts.each(function (evt) {
            var M = Ext.calendar.EventMappings,
             st = new Date(evt.data.StartDate),
             end = new Date(evt.data.EndDate),
            days = Ext.calendar.Date.diffDays(
            Ext.calendar.Date.max(this.viewStart, st),
            Ext.calendar.Date.min(this.viewEnd, end)) + 1;

            if (days > 1 || Ext.calendar.Date.diffDays(st, end) > 1) {
    ...
    isEventVisible: function (evt) {
       ...
       st = new Date(evt.StartDate),
       en = new Date(evt.EndDate),
        evStart = st.getTime(), //(evt.data ? evt.data[M.StartDate.name] : evt[M.StartDate.name]).getTime(),
        evEnd = en.getTime(),//(evt.data ? evt.data[M.EndDate.name] : evt[M.EndDate.name]).add(Date.SECOND, -1).getTime(),
